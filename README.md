...
ローカルお気に入り登録、通知対象設定、グローバルな入室/退室通知切替、フィルタ、入室導線を提供します。
...
- お気に入りユーザ / 通知対象ユーザのローカル管理（`localStorage`）
- 表示フィルターと自動更新間隔の永続化（`localStorage`）
- Web Notifications API による通知
  - メンバー行の `🔔` で通知対象ユーザを登録
  - ツールバー下段の `入室通知` / `退室通知` で通知種別を全体切替
  - 入室通知: 同一セッション中は `(userId, roomId)` ごとに一度だけ
  - 退室通知: 差分検知ごとに毎回通知
  - `Official Test Room` は通知対象外
- ルームカードの見やすさ改善
  - 鍵ありルームは左側に `🔒` アイコン表示 + やや濃いグレー背景
  - タイトル行は `部屋名 / 人数 / 入室ボタン` を 1 行で表示
  - オーナー表示は独立テキストではなく、メンバー行をライトブルーで強調
...
- フィルタ
  ...
  - テストルーム表示
- `入室` ボタンによる外部アプリ起動導線
  - `https://webapi.syncroom.appservice.yamaha.com/launch_app?...` を新規タブで開く
...
- 通知対象にしたいメンバーは、各メンバー行の **`🔔` ボタン**で切り替えます。
- ツールバーの **`入室通知` / `退室通知`** は、イベント種別の全体 ON/OFF です。
...
- お気に入り設定・通知対象設定・表示フィルター・自動更新間隔は `localStorage` に保存されます
- 表示順は現在 `通知対象ルーム → お気に入りルーム → その他` を優先する固定です
- `launch_app` リンクはブラウザ側のポップアップ制限や外部アプリ起動制限の影響を受ける場合があります
